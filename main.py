...
async def backtest(days: int = 7):
    ...
    message = f"Backtest results for last {days} days (from {start_date}): Simulated high-conviction positive FVGs found - details in logs. Top setups: [list from simulation, e.g., MU, ORCL]. Average pop potential: 12.5%. Rollover alerts tested on owned stocks/sector outflows."
    async with httpx.AsyncClient() as client:
        await client.post(DISCORD_WEBHOOK, json={"content": message})
...
if __name__ == "__main__":
    ...
    # For server mode: uvicorn.run(app, host="0.0.0.0", port=8000)
...
async def send_discord(message: str):
    ...
    async with httpx.AsyncClient() as client:
        try:
            await client.post(DISCORD_WEBHOOK, json={"content": message[:1995]}, timeout=10.0)
        except Exception:
            pass
...
async def get_ohlc(symbol: str, interval: str = '1h', limit: int = 250):
    async with httpx.AsyncClient() as client:
        try:
            ...
            params = {"limit": limit}
            resp = await rate_limited_get(client, url, params=params, headers=HEADERS, timeout=20.0)
            ...
            if now - last_error_time > 3600:
                await send_discord(f"OHLC error on {symbol}: {str(e)}")
                last_error_time = now
            return None
...
async def get_flow(symbol: str):
    async with httpx.AsyncClient() as client:
        try:
            resp = await rate_limited_get(client, f"{BASE_URL}/flow/today", params={"ticker": symbol.upper()}, headers=HEADERS, timeout=20.0)
            resp.raise_for_status()
            return resp.json()
        except Exception:
            try:
                resp = await rate_limited_get(client, f"{BASE_URL}/flow/alerts", params={"ticker": symbol.upper(), "limit": 50}, headers=HEADERS, timeout=20.0)
                resp.raise_for_status()
                return resp.json()
            except Exception:
                return None
...
def get_whale_premium(flow_data):
    if not flow_data or 'data' not in flow_data or not flow_data['data']:
    ...
    above_ask_count = 0
    multi_ex_count = 0
    spot = flow_data.get('spot_price') or (flow_data['data'][0].get('spot_price') if flow_data['data'] else 0)
    
    for trade in flow_data['data']:
        prem = trade.get('premium') or trade.get('total_premium', 0)
        strike = trade.get('strike') or trade.get('strike_price', 0)
        is_call = trade.get('is_call') or (trade.get('option_type', '').lower() == 'call')
        
        if is_call:
            premium_call += prem
            if strike > spot * 1.02:
                otm_call_prem += prem
                typ = trade.get('type', '').lower()
                if 'sweep' in typ or 'block' in typ:
                    otm_sweep_count += 1
        if trade.get('above_ask') or trade.get('sent_at_ask') or 'above_ask' in trade.get('trade_type', '').lower():
            above_ask_count += 1
        if ',' in str(trade.get('exchange', '')):
            multi_ex_count += 1
...
async def get_macro_context():
    ...
    async with httpx.AsyncClient() as client:
        for sym, name in symbols.items():
            try:
                resp = await rate_limited_get(client, f"https://api.polygon.io/v2/last/trade/{sym}", params={"apiKey": MASSIVE_API_KEY})
                resp.raise_for_status()
                price = resp.json().get('last', {}).get('price', 'N/A')
                context += f"{name} ${price:.2f} | " if isinstance(price, (float, int)) else f"{name} N/A | "
            except Exception:
                context += f"{name} N/A | "
...
async def scheduler():
    await send_discord("BEAST RELOADED 🦁🔥\nFixes: Rate limiting + error suppression + reliable endpoints + dedupe + stable flow parsing.\nClean accurate live data now locked in.")
    while True:
        try:
            now = datetime.now(ZoneInfo("America/Chicago"))
            hour = now.hour
            ...
            # Regular hours: 8:30–15:00 CT → every 15 minutes
            elif 8 <= hour < 15:
                if minute % 15 == 0:
                    await fvg_whale_scan()
                    await daily_pre_market_summary()
            ...
            global last_error_time
            now_t = time.time()
            if now_t - last_error_time > 3600:
                await send_discord(f"Major scanner error (suppressed repeats): {str(e)}")
